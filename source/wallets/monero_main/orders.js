...
const xmr = require("../monero_main/utils")
...
exports.HandleCreateOrder = async function(params)
{
    ...
    try {
        ...
        const balance = await xmr.GetBalance(order.addressMonero);
...
{
    const rows = await g_constants.dbTables["orders"].Select("SUM(sell_amount) AS balanceInOrders", "seller_pubkey='"+escape(pubkey)+"' AND sell_coin='xmr'")
